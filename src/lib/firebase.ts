...
// Use the named database if provided, otherwise use the default database
...
async function testConnection() {
  try {
    ...
    if (error instanceof Error && (error.message.includes("the client is offline") || error.message.includes("Could not reach Cloud Firestore backend"))) {
      console.error("Firestore Connection Error: Please check your Firebase configuration and internet connection.");
    }
  }
}
testConnection();
